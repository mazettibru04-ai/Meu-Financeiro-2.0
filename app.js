...
window.addProduto = async function() {
  ...
  const venda     = Number(document.getElementById("p-venda").value);
  const moeda     = document.getElementById("p-moeda").value;
  const estoque   = Number(document.getElementById("p-estoque").value);

  if (!nome || custo <= 0 || venda <= 0 || estoque < 0) {
    return alert("Preencha corretamente os campos");
  }

  if (venda < custo) {
    if (!confirm("Preço de venda menor que custo. Deseja continuar?")) return;
  }
...
// =====================
function calcularInvestimentoProdutos(lista) {
  totalInvestidoProdutos = 0;

  lista.forEach(p => {
    const custoEUR = eur(p.precoCusto, p.moeda);
    totalInvestidoProdutos += custoEUR * p.estoque;
  });

  document.getElementById("total-produtos").innerText =
    `€ ${fmt(totalInvestidoProdutos)}`;
}
...
function iniciarStreamProdutos() {
  ...
  const u = onSnapshot(q, (snap) => {
    ...
    snap.forEach((i) => {
      ...
      const custoEUR = eur(p.precoCusto, p.moeda);
      const vendaEUR = eur(p.precoVenda, p.moeda);

      const margem = ((vendaEUR - custoEUR) / custoEUR) * 100;
      ...
      const estoqueBadge =
        p.estoque <= 5
          ? `<span style="color:#ef4444">🔴 Baixo (${p.estoque})</span>`
          : `<span style="color:#22c55e">🟢 ${p.estoque}</span>`;

      html += `
        <div class="card">
          <strong>${p.nome}</strong>
          <p>${p.categoria}</p>
          <p>Custo: ${p.moeda} ${fmt(p.precoCusto)}</p>
          <p>Venda: ${p.moeda} ${fmt(p.precoVenda)}</p>
          <p style="color:${corMargem}">
            Margem: ${margem.toFixed(1)}%
          </p>
          <p>Estoque: ${estoqueBadge}</p>

          <div class="actions">
            <button class="btn-remover"
              onclick="deletarItem('produtos','${i.id}','${p.nome}')">
              🗑️ Remover
            </button>
          </div>
        </div>
    ...
    calcularInvestimentoProdutos(lista);
  });
