...
// =====================
// DELETAR DÍVIDA (específico - pega desc do cache)
// =====================
window.deletarDivida = async function(id) {
  const data = recuperar(id);
  if (!data) return alert("Dívida não encontrada");
  
  if (!confirm(`Remover "${data.desc}"?\n\nEssa ação não pode ser desfeita.`)) return;

  const ref = getDoc("dividas", id);
  if (!ref) return;

  await deleteDoc(ref);
  await registrarHistorico("🗑️ Dívida removida", data.desc, "-", "-");
};
      ...
      html += `
        <div class="card ${quitada ? "pago-total" : ""}">
          ...
          <div class="actions">
            ...
            ` : ""}
            <button class="btn-remover" onclick="deletarDivida('${i.id}')">🗑️ Remover</button>
          </div>
        </div>`;
    });
