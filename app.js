...
window.addReceita = async function () {
  const btn = document.querySelector("[onclick=\"addReceita()\"]");
  try {
    ...
    await addDoc(collection(db, "receitas"), { desc, val, criadoEm: Date.now() });
...
window.addDespesa = async function () {
  const btn = document.querySelector("[onclick=\"addDespesa()\"]");
  try {
    const desc = document.getElementById("d-desc").value.trim();
    const val = Number(document.getElementById("d-val").value);
    if (!desc || isNaN(val) || val <= 0) { alert("Preencha corretamente"); return; }
    if (btn) btn.disabled = true;
    await addDoc(collection(db, "despesas"), { desc, val, criadoEm: Date.now() });
...
window.addDivida = async function () {
  ...
  if (!desc || isNaN(valor) || valor <= 0) { alert("Preencha corretamente"); return; }
  await addDoc(collection(db, "dividas"), {
    desc, valorOriginal: valor, pago: 0, criadoEm: Date.now()
  });
